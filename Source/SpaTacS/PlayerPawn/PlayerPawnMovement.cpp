...
UPlayerPawnMovement::UPlayerPawnMovement(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	...
	ReferenceVelocity = {0.f, 0.f, 0.f};

	ResetMoveState();
}

void UPlayerPawnMovement::TickComponent(float DeltaTime, enum ELevelTick TickType, FActorComponentTickFunction *ThisTickFunction)
{
	...
	DeltaTime = bIgnoreTimeDilation ? FApp::GetDeltaTime() : DeltaTime;
	...
	if (Controller && Controller->IsLocalController())
	{
		...
		// Move actor
		FVector Delta = (Velocity + ReferenceVelocity) * DeltaTime;
...
bool UPlayerPawnMovement::LimitWorldBounds()
{
...
void UPlayerPawnMovement::ApplyControlInputToVelocity(float DeltaTime)
{
...
bool UPlayerPawnMovement::ResolvePenetrationImpl(const FVector& Adjustment, const FHitResult& Hit, const FQuat& NewRotationQuat)
{
