...
void PowerProduction::operator()(const PowerPlantData& pp, const Health& health, const Activity& acc, SubsystemManager& entities)
{
    auto base_energy = 0.1_s * pp.energy_production * acc.get();
    mass_t need_fuel = base_energy * 10.0_kg / 1.0_MJ;

    mass_t fuel = request_fuel(entities, need_fuel);
